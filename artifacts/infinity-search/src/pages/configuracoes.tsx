...
import { ShieldAlert, UserPlus, Trash2, LogOut, User as UserIcon, Crown, Calendar, Shield, Clock, X, Check, Bell, Send, KeyRound, Eye, EyeOff, Hash, RefreshCw, Lock, Pencil, RotateCcw, ImagePlus, Loader2, CreditCard, CheckCircle2, XCircle, AlertCircle } from "lucide-react";

// ─── Pending Accounts Admin Section ──────────────────────────────────────────
...
function PendingAccountsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch("/api/infinity/pending-accounts", {
  ...
  const approve = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("infinity_token");
      const r = await fetch(`/api/infinity/pending-accounts/${id}/approve`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token ?? ""}` },
      });
      if (!r.ok) {
        const j = await r.json().catch(() => ({})) as { error?: string };
        setActionErr(prev => ({ ...prev, [id]: j.error ?? "Erro ao aprovar" }));
        return;
      }
      await load();
    } catch { setActionErr(prev => ({ ...prev, [id]: "Falha na conexão" })); }
  ...
  const reject = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("infinity_token");
      const r = await fetch(`/api/infinity/pending-accounts/${id}/reject`, {
        method: "POST",
      ...
      if (!r.ok) {
        const j = await r.json().catch(() => ({})) as { error?: string };
        setActionErr(prev => ({ ...prev, [id]: j.error ?? "Erro ao rejeitar" }));
        return;
      }
      await load();
    } catch { setActionErr(prev => ({ ...prev, [id]: "Falha na conexão" })); }
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-amber-400/20 bg-amber-400/5 backdrop-blur-2xl p-6"
    >
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground">
            ...
            {accounts.length > 0 && (
              <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-amber-400/15 border border-amber-400/30 text-amber-300 font-bold">
                {accounts.length}
              </span>
            )}
          </h2>
      ...
      {accounts.length === 0 ? (
        <div className="rounded-xl border border-white/5 bg-black/20 px-4 py-8 text-center text-xs text-muted-foreground">
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    <div className="font-bold text-sm flex items-center gap-2">
                      @{a.username}
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-amber-400/10 border border-amber-400/20 text-amber-300 uppercase tracking-widest font-bold">
                        {PLAN_LABELS[a.planId] ?? a.planId}
                      </span>
                    </div>
                    {a.email && (
                    ...
                    <div className="text-[10px] text-muted-foreground/50 mt-1">
                      Solicitado: {new Date(a.createdAt).toLocaleString("pt-BR")}
                    </div>
                    {actionErr[a.id] && (
                      <div className="text-[10px] text-destructive mt-1 flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" /> {actionErr[a.id]}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-400/10 border border-emerald-400/30 text-emerald-300 text-xs font-bold uppercase tracking-widest hover:bg-emerald-400/20 transition-all disabled:opacity-50"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      {actionPending === a.id ? "..." : "Aprovar"}
                    </button>
                    <button
                      onClick={() => void reject(a.id)}
                      disabled={actionPending === a.id}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-destructive/10 border border-destructive/30 text-destructive text-xs font-bold uppercase tracking-widest hover:bg-destructive/20 transition-all disabled:opacity-50"
                    >
